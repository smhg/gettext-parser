  ...
  for (let i = 0, len = chunk.length; i < len; i++) {
    ...
    switch (this._state) {
      ...
      case this.states.obsolete:
        ...
        } else if (chr === '|' && this._state === this.states.obsolete) {
          // "#~|": previous string of an obsolete entry
          this._node = {
            type: this.types.comments,
            value: chr
          };
        ...
        if (this._escaped) {
          switch (chr) {
            ...
            case 'r':
              ...
              break;
            case '\n':
              // a backslash at the end of the line continues the string on the next line
              break;
            default:
              this._node.value += chr;
          }
          ...
          if (chr === this._node.quote) {
            ...
            const err = new SyntaxError(`Error parsing PO data: End-of-line within string at line ${this._lineNumber}. A string spanning multiple lines must be split into separately quoted lines, with "\\n" to encode the line breaks.`);
 ...
 * @param {number} nplurals Number of expected plural forms, 0 when the headers do not declare it
...
Parser.prototype._validateToken = function (
  ...
  } else if (msgid_plural && !nplurals) {
    throw new SyntaxError(`Plural forms error: entry "${msgid}" in "${msgctxt}" context has plural forms but no valid "nplurals" value was found in the "Plural-Forms" header.`);
  } else if (msgid_plural && msgstr.length !== nplurals) {
    throw new RangeError(`Plural forms range error: Expected to find ${nplurals} forms but got ${msgstr.length} for entry "${msgid}" in "${msgctxt}" context.`);
  } else if (!msgid_plural && msgstr.length !== 1) {
    throw new RangeError(`Translation string range error: Expected 1 msgstr definitions associated with "${msgid}" in "${msgctxt}" context, found ${msgstr.length}.`);
  }
};
...
Parser.prototype._normalize = function (tokens) {
  ...
  // 0 until the "Plural-Forms" header declares a usable "nplurals" value
  let nplurals = 0;
  let msgctxt;
...
Parser.prototype._finalize = function (tokens) {
  if (this._validation && this._state === this.states.string) {
    const err = new SyntaxError(`Error parsing PO data: End-of-file within string at line ${this._lineNumber}`);
