...
/**
* Generate a PO object transform Stream
*
* @param {String} [defaultCharset] Default charset to use
* @return {stream.Transform} A stream.Transform in object mode
*/
module.exports.stream = function(defaultCharset){
  
    var parser, opts = {};
    
    function transform(chunk, enc, callback){
      
        // Init the parser with the first chunk
        if (!parser) parser = new Parser(chunk, defaultCharset);
        
        opts = parser._lexer(parser._bufferToString(chunk), opts);
        callback();
    };
    
    function flush(callback){
      
        var lex = parser._joinStringValues(opts.lex);
        parser._parseComments(lex);
        lex = parser._handleKeys(lex);
        lex = parser._handleValues(lex);
        
        this.push(parser._normalize(lex));
...
Parser.prototype._handleCharset = function(buf) {
    ...
    if (this._charset === 'utf-8') {
        ...
        this._fileContents = this._bufferToString(buf);
    }
};
 ...
 *
 * @param {Buffer} A buffer to be converted
 * @return {string} The buffer converted as an `utf-8` string
 */
Parser.prototype._bufferToString = function(buf){
 ...
 * Token parser
 * 
 * @param {string} the chunk string to be parsed
 * @param {opts} optional previous state including `lex`, `node` and `state`
 * @return {Object} Parsed tokens `lex`, last `state` and `node`
 */
Parser.prototype._lexer = function(chunk, opts) {
    var opts = opts || {},
        chr,
        escaped = false,
        lex = opts.lex || [],
        node = opts.node,
        state = opts.state || this.states.none;
...
Parser.prototype.parse = function() {
    var lex = this._lexer(this._fileContents).lex;
